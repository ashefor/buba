...
export class RaffleDrawComponent implements OnInit, OnDestroy {
  ...
  constructor(private service: GamesService, private toastr: ToastrService, private loadingBar: LoadingBarService, private title: Title) {
  ...
  fetchSpinItems() {
    ...
    this.fetchSubscription = this.service.fetchSpinItems().subscribe((data: any) => {
      ...
      if (data.status === 'success') {
        this.spinData = data;
        this.entries = data.product?.sort((a, b) => a.product_name.localeCompare(b.product_name));
      }
    }, (error: any) => {
      this.loadingBar.stop();
      this.loadingDetails = false;
  ...
  startSpin(raffle: HTMLElement) {
    ...
    this.spinSubscription = this.service.startSpinSession(spinDetails).subscribe((data: any) => {
      ...
      raffle.addEventListener('animationend', () => {
        ...
        } else if (this.retryData.spin_status === 1 || this.retryData.spin_status === 2 || this.retryData.spin_status === 3) {
          this.toastr.success('You have won. Please check dashboard/spin history for more details');
        }
